...
function Home() {
    ...
    useEffect(() => {
        fetch(
            `${UMBRACO_API_URL}/?fetch=children:posts&sort=updateDate:desc&take=10&expand=properties[coverImage,author[properties[picture]]]&fields=properties[excerpt,tags,coverImage,author[properties[picture]]]`,
            {
                method: 'GET',
                headers: {
                    'Start-Item': 'home',
                        ...
                        { firstPost &&
                            <div className="flex h-full bg-white rounded overflow-hidden shadow-lg" style={{minHeight: '50vh'}}>
                                <div className="flex flex-wrap no-underline hover:no-underline w-full">
                                    <Link to={postUrl(firstPost)} className="w-full md:w-2/3 rounded-t" style={{display: 'flex', alignItems: 'center'}}>
                                        <img src={imageUrl(firstPost.properties.coverImage[0])} className="w-full" alt={imageAltText(firstPost.properties.coverImage[0])}></img>
                                    </Link>
                                    ...
                                    <div className="w-full md:w-1/3 flex flex-col flex-grow flex-shrink">
                                        <div className="flex-1 bg-white rounded-t rounded-b-none overflow-hidden shadow-lg">
                                            ...
                                            <p className="w-full text-gray-600 text-xs md:text-sm px-6"><PostTags post={firstPost} /></p>
                                            <p className="text-gray-800 font-serif text-base px-6 pt-6 mb-5">{firstPost.properties.excerpt}</p>
                                        </div>

                                        <Link to={authorUrl(firstPost.properties.author)}
                                              className="flex-none mt-auto bg-white rounded-b rounded-t-none overflow-hidden shadow-lg p-6">
                                            <div className="flex items-center">
                                                <img className="w-8 h-8 rounded-full mr-4 avatar" src={imageUrl(firstPost.properties.author.properties.picture[0])} alt={imageAltText(firstPost.properties.author.properties.picture[0])}></img>
                            ...
                            <div className="flex flex-wrap justify-between pt-12 -mx-6">
                                {otherPosts.map(post =>
                                    <div className="w-full md:w-1/2 p-6 flex flex-col flex-shrink" key={post.id}>
                                        ...
                                        <Link to={authorUrl(post.properties.author)} className="flex-none mt-auto bg-white rounded-b rounded-t-none overflow-hidden shadow-lg p-6">
                                            <div className="flex items-center">
                                                <img className="w-8 h-8 rounded-full mr-4 avatar" src={imageUrl(post.properties.author.properties.picture[0])} alt={imageAltText(post.properties.author.properties.picture[0])}></img>
