...
function Tag() {
    ...
    useEffect(() => {
        // fetch posts tagged with the tag from the route
        // - note that we're using our custom "tag" filter to find the posts
        fetch(
            `${UMBRACO_API_URL}/?fetch=children:posts&filter=tag:${tag}&sort=updateDate:desc&take=10&expand=properties[coverImage,author[properties[picture]]]&fields=properties[excerpt,tags,coverImage,author[properties[picture]]]`,
            {
                method: 'GET',
                headers: {
                    'Start-Item': 'home',
